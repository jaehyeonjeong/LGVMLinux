...
//extern int readBMP(char *filename, BYTE **pData, int *cols, int *rows, int *color);

void cvtBGR2Gray(BYTE *dest, BYTE  *src, int n)
{
	for(int i = 0; i < n; i++){
		int b = *src++;
		int g = *src++;
...
//	int r, g, b;	//픽셀 r, g, b원소 정의
	...
	BYTE r, g, b, a = 255;
	BYTE *pImageData, *pData, *pBmpData, pFbMap;
	...
	pData = (BYTE*)malloc(vinfo.xres * vinfo.yres * sizeof(BYTE) * color / 8);
	pImageData = (BYTE*)malloc(vinfo.xres * vinfo.yres * sizeof(BYTE) * color / 8);
	...
	fwrite(pData, sizeof(BYTE), imagesize, copyfp);				//복사파일을 이미지 사이즈 호출
